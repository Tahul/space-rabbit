...
| `make background` | Regenerates `Dmg/Background.tiff` from `Dmg/CreateBackground.swift` |
...
4. `osascript Dmg/Layout.applescript` drives Finder to set the window bounds, icon size,
...
the Makefile and the matching constants at the top of `Dmg/CreateBackground.swift`. The
...
**Custom artwork:** the generated background is a brand-neutral placeholder. Point the
Makefile at a real asset with `make dmg DMG_BACKGROUND=path/to/art.tiff` (or replace
`Dmg/CreateBackground.swift`). Any replacement must be 700×460 pt and keep the arrow in
the icon gap. A *light* background is deliberate: Finder draws icon labels in the system
appearance, so dark artwork makes them unreadable for light-mode users.
...
Dmg/
  Background.tiff       — installer artwork (git-ignored, regenerated by `make background`)
