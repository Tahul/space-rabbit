...
1. **Drop in a PNG** (the normal path) — put a **1400×920 px** file (the @2x of the
   ...
   `Tools/Dmg/BaseBackground.png`. `make background` picks it up automatically via the
   ...
   stretch. Use a different path with `make background DMG_BACKGROUND_PNG=path/to/art.png`.
...
Tools/                  — build-time asset generators (not compiled into the app)
  ...
  Dmg/
    ...
    Background.tiff     — final installer artwork (git-ignored, `make background`)
